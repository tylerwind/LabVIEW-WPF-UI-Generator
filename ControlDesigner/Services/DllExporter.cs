...
namespace ControlDesigner.Services
{
    ...
    public class DllExporter
    {
        ...
        /// 导出包含所有控件的统一 DLL
        /// </summary>
        public ExportResult ExportAll(ControlStyle style, string outputDllPath, string assemblyName)
        {
            var result = new ExportResult();
            ...
            try
            {
                ...
                string tempDir = Path.Combine(Path.GetTempPath(),
                    "LvControlExport_" + Guid.NewGuid().ToString("N").Substring(0, 8));
                _templateEngine.GenerateAllProject(style, tempDir, assemblyName);

                // 2. 调用 MSBuild
                string csproj = Path.Combine(tempDir, assemblyName + ".csproj");
                var psi = new ProcessStartInfo
                {
                    FileName = _msBuildPath,
                    Arguments = string.Format("\"{0}\" /p:Configuration=Release /verbosity:minimal /t:Rebuild", csproj),
                ...
                if (result.ExitCode == 0)
                {
                    string builtDll = Path.Combine(tempDir, "bin", "Release", assemblyName + ".dll");
                    if (File.Exists(builtDll))
                    {
                        string outDir = Path.GetDirectoryName(outputDllPath);
                        if (!string.IsNullOrEmpty(outDir))
                        ...
                        result.Success = true;
                        result.DllPath = outputDllPath;
                        result.ControlName = assemblyName;
                    }
                    else
                    {
                        result.ErrorMessage = "编译成功但未找到输出 DLL: " + builtDll;
        ...
        private string FindMsBuild()
        {
            string[] candidates = {
                ...
                // 增加系统 4.0 框架路径作为终极保底 (对方没有 VS 时生效)
